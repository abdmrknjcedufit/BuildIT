...
namespace BuildIT.API.Controllers
{
    ...
    public class ListingController : BaseCRUDController<Model.Models.Listing, ListingSearchObject, ListingInsertRequest, ListingUpdateRequest>
    {
        ...
        public ListingController(IListingService service, BuildITDbContext context, IRecommenderService recommenderService) : base(service)
        {
            _service = service;
            _context = context;
        ...
        [AllowAnonymous]
        public ActionResult<List<Model.Models.Listing>> GetRecommendations(int userId, [FromQuery] int count = 10)
        {
            try
            {
                var recommendations = _recommenderService.GetRecommendations(userId, count);
                return Ok(recommendations);
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = $"Greška pri dohvatanju preporuka: {ex.Message}" });
            }
        }
    }
}
