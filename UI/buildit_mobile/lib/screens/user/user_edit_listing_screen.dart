...
class UserEditListingScreen extends StatefulWidget {
  final Listing listing;
  
  const UserEditListingScreen({super.key, required this.listing});

  @override
  State<UserEditListingScreen> createState() => _UserEditListingScreenState();
}

class _UserEditListingScreenState extends State<UserEditListingScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      setState(() {
        ...
        _selectedItemId = widget.listing.itemId;
        _selectedListingType = widget.listing.listingType;
        _selectedCityId = widget.listing.cityId;
        _isFeatured = widget.listing.isFeatured;
        _titleController.text = widget.listing.title;
        _descriptionController.text = widget.listing.description;
        if (widget.listing.minRentalDays != null) {
          _minRentalDaysController.text = widget.listing.minRentalDays.toString();
        }
        if (widget.listing.maxRentalDays != null) {
          _maxRentalDaysController.text = widget.listing.maxRentalDays.toString();
        }
  ...
  Future<void> _saveListing() async {
    ...
    try {
      ...
      if (_selectedItemId != widget.listing.itemId) {
        updateRequest['itemId'] = _selectedItemId;
      }

      if (_selectedListingType == 'Iznajmljivanje') {
        if (_minRentalDaysController.text.trim().isNotEmpty) {
          updateRequest['minRentalDays'] = int.tryParse(_minRentalDaysController.text.trim());
        }
        if (_maxRentalDaysController.text.trim().isNotEmpty) {
          updateRequest['maxRentalDays'] = int.tryParse(_maxRentalDaysController.text.trim());
        }
      } else {
        updateRequest['minRentalDays'] = null;
        updateRequest['maxRentalDays'] = null;
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Greška pri ažuriranju oglasa: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
