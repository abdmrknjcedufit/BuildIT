...
namespace BuildIT.Services.Services
{
    public class OrderNotificationService : BackgroundService
    {
        ...
        private async Task ConnectToRabbitMQ(string hostName, int port, string userName, string password, CancellationToken cancellationToken)
        {
            try
            {
        ...
        private async Task<string?> GetSellerEmailAsync(int sellerId)
        {
            try
            {
                ...
                _logger.LogInformation($"🔵 OrderNotificationService: Email prodavca dohvaćen za UserId {sellerId}: {(email ?? "N/A")}");
