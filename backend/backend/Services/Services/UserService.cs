...
namespace BuildIT.Services.Services
{
    public class UserService : BaseCRUDService<Model.Models.User, UserSearchObject, Database.User, UserInsertRequest, UserUpdateRequest>, IUserService
    {
        ...
        public async Task<string> ChangePasswordAsync(int userId, string oldPassword, string newPassword, string newPasswordConfirm)
        {
            var user = Context.Users.FirstOrDefault(u => u.Id == userId);
        ...
        public async Task<string> EnableTwoFactorAsync(int userId, string method)
        {
            ...
            return "Dvofaktorska autentifikacija je uspješno omogućena.";
        }

        public async Task<string> DisableTwoFactorAsync(int userId)
        {
            var user = Context.Users.FirstOrDefault(u => u.Id == userId);
            if (user == null)
                throw new UserException("Korisnik nije pronađen.");
            ...
            return "Dvofaktorska autentifikacija je uspješno onemogućena.";
        }
